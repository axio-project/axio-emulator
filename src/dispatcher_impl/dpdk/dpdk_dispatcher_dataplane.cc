...
size_t DpdkDispatcher::pre_dispatch_pkts(){
  size_t pre_dispatch_total = 0;

  /// echo received packets, mock the behaviour of OvS
  auto __echo_handler = [&]() -> size_t {
    rte_mbuf *mbuf;
    struct eth_hdr *eth = NULL;
    struct iphdr *iph = NULL;
    ...
    for (size_t i = pre_dispatch_total; i < rx_queue_idx_; i++)
  ...
  if constexpr (handler == kRxDispatcherHandler_Echo){
    return __echo_handler();
  } else {
    return pre_dispatch_total;
  }
}
...
// force compile
template size_t DpdkDispatcher::pre_dispatch_pkts<kRxDispatcherHandler>();
